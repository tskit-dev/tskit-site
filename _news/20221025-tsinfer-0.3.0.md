...
layout: news
title: Tsinfer 0.3.0!
description: Tsinfer 0.3.0 introduces a post-processing step that improves time
...
than requiring the ancestral allele to be first. Finally, changes are introduced to make
it less easy to carry out inappropriate analyses (e.g. by setting time units and by
...
When creating a tsinfer input file, instead of the ancestral state being the first allele in the list, with value 0 in the associated genotypes array, you can specify an "ancestral_allele", giving
 an alternative index into the alleles list. By default this is 0 (maintaining backwards compatibility) but `tskit.MISSING_DATA` can be specified, which places mutations at that site using parsimony, inferring the most parsimonious ancestral state.
...
![png](/assets/images/news/20221025-tsinfer-0.3/20221025-tsinfer-0.3.0_20_0.png)
